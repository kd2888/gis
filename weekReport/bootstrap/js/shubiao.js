...
(function() {
     ...
     * 判断是pc还是移动端
     */
    DrawingManager.prototype._isPC=function()
    {
    ...
    DrawingManager.prototype._bindPolylineOrPolygon = function() {
        ...
        var dblclickAction = function (e) {
            ...
            mask.removeEventListener('touchstart',startAction);
           // mask.removeEventListener('mousemove', mousemoveAction);
            ...
            me.mousemoveAction=mousemoveAction;
        ...
        if(me._isPC()){
            ...
            map.disableDragging();
            console.log( $('.BMap_mask'))
            $('.BMap_mask').css("z-index","900")
            $('.BMap_mask').on('touchend', function (e) {
                console.log(e)
                startAction(e)
            });
        }
     ...
     * 移除鼠标事件(只对画多边形有效)
     */
    DrawingManager.prototype._bindRemove=function () {
        var me           = this,
            map          = this._map,
            mask         = this._mask;
        isBinded = false;
        if(mask.disableEdgeMove){
            mask.disableEdgeMove();
        }
        if(me.mousemoveAction!=""){
            if(me._isPC()){
                mask.removeEventListener('mousedown', startAction);
            }else{
                $('.BMap_mask').unbind('touchmove');
                map.enableDragging();
            }
            me.mousemoveAction="";
        }
        if(me.mousemoveAction!=""){
            mask.removeEventListener('touchend', me.mousemoveAction);
            me.mousemoveAction="";
        }

        if(me.drawing!=""){
            map.removeOverlay(me.drawing);
            me.drawing="";
        }
        me.close();
    }
